...
describe('TownServiceApiSocket', () => {
  ...
  it('Rejects invalid CoveyTownIDs, even if otherwise valid session token', async () => {
    ...
    const joinData = await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: nanoid(), carType: 'REGULAR_GREEN' });
  ...
  it('Rejects invalid session tokens, even if otherwise valid town id', async () => {
    const town = await createTownForTesting();
    await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: nanoid(), carType: 'REGULAR_GREEN' });
  ...
  it('Dispatches movement updates to all clients in the same town', async () => {
    const town = await createTownForTesting();
    const joinData = await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: nanoid(), carType: 'REGULAR_GREEN' });
    const joinData2 = await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: nanoid(), carType: 'REGULAR_GREEN' });
    const joinData3 = await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: nanoid(), carType: 'REGULAR_GREEN' });
  ...
  it('Informs all new players when a player joins', async () => {
    ...
    await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: newJoinerName, carType: 'REGULAR_GREEN' });
  ...
  it('Informs all players when a player disconnects', async () => {
    ...
    const joinDataWhoLeaves = await apiClient.joinTown({ coveyTownID: town.coveyTownID, userName: userWhoLeaves, carType: 'REGULAR_GREEN' });
